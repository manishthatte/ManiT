...
Parser::Parser(Lexer& l) : lexer(l) {
    precedences = {
        {TokenType::EQUAL, Precedence::ASSIGN}, {TokenType::EQUAL_EQUAL, Precedence::EQUALS},
        {TokenType::BANG_EQUAL, Precedence::EQUALS}, {TokenType::LESS, Precedence::LESSGREATER},
        {TokenType::GREATER, Precedence::LESSGREATER}, {TokenType::LESS_EQUAL, Precedence::LESSGREATER},
        {TokenType::GREATER_EQUAL, Precedence::LESSGREATER}, {TokenType::PLUS, Precedence::SUM},
        {TokenType::MINUS, Precedence::SUM}, {TokenType::SLASH, Precedence::PRODUCT},
        {TokenType::STAR, Precedence::PRODUCT}, {TokenType::LPAREN, Precedence::CALL},
    };
    next_token();
    next_token();
}
...
std::unique_ptr<Statement> Parser::parse_statement() {
    switch (current_token.type) {
        ...
        case TokenType::VAR: return parse_var_statement();
...
std::unique_ptr<Expression> Parser::parse_integer_literal() {
    ...
    if (result.ec != std::errc() || result.ptr != s.data() + s.size()) return nullptr;
    return literal;
}
...
// **FIXED FUNCTION**
std::unique_ptr<Expression> Parser::parse_for_loop_expression() {
    auto expr = std::make_unique<ForLoopExpression>();
    expr->token = current_token;

    if (peek_token.type != TokenType::LPAREN) return nullptr;
    next_token(); // on (
    next_token(); // on initializer start

    // --- Initializer ---
    if (current_token.type != TokenType::SEMICOLON) {
        expr->initializer = parse_statement();
        // parse_statement leaves current_token ON the semicolon.
    }
    if (current_token.type != TokenType::SEMICOLON) return nullptr;
    next_token(); // consume ;

    // --- Condition ---
    if (current_token.type != TokenType::SEMICOLON) {
        expr->condition = parse_expression(Precedence::LOWEST);
        // after parse_expression, current_token is the LAST token of the expr.
    }
    if (peek_token.type != TokenType::SEMICOLON) return nullptr;
    next_token(); // Advance to the semicolon
    next_token(); // Consume the semicolon

    // --- Increment ---
    if (current_token.type != TokenType::RPAREN) {
        expr->increment = parse_expression(Precedence::LOWEST);
        // after parse_expression, current_token is the LAST token of the expr.
    }
    if (peek_token.type != TokenType::RPAREN) return nullptr;
    next_token(); // Advance to the ')'
